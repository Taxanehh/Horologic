...
// Only show watches with the "Repair complete" label.
$statusTags = [
    'Reparatie klaar' => ['label' => 'Repair complete', 'class' => 'repa-box']
];

$conn = getDbConnection();
$stmt = $conn->prepare("SELECT * FROM horloges WHERE Tag = 'Reparatie klaar' ORDER BY ReparatieNummer DESC");
$stmt->execute();
    ...
    <main style="padding: 1rem;">
        ...
        <div style="text-align: center; margin: 2rem 0; padding: 1rem; background-color: #f0f8ff; border-radius: 8px; border: 2px solid #153c63;">
            <h2 style="margin: 0; color: #153c63; font-size: 1.5rem;">
                🢂 Select a watch to create an invoice 🢀
            </h2>
            <p style="margin: 0.5rem 0 0 0; color: #2a6496; font-size: 1rem;">
                Click on any item below to view details and generate an invoice
            </p>
            <p style="margin: 0.5rem 0 0 0; color: #2a6496; font-size: 1rem;">
                Only watches labeled "Repair complete" are eligible for invoicing
            </p>
        </div>
        ...
        <div class="horloge-list">
            ...
            <div class="horloge-item repa-box" 
                ...
                <div class="horloge-tag">
                    <span class="tag-label repa-box">Repair complete</span>
                </div>
            </div>
            <?php endforeach; ?>
        </div>
        ...
        <script>
            ...
            function showInvoiceModal(repairId) {
            ...
            document.querySelectorAll('.horloge-item').forEach(item => {
                item.addEventListener('click', function(e) {
                    ...
                    showInvoiceModal(repairId);
                });
            });

            // Filter functions (identical to reparaties.php)
